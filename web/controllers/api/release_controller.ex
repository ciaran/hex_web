defmodule HexWeb.API.ReleaseController do
  ...
  def delete(conn, _params) do
    ...
    multi =
      ...
      |> Ecto.Multi.run(:log, fn _ -> audit(conn, "release.revert", {release.package, release}) end)
  ...
  defp create(conn, package, params, checksum, user, body) do
    ...
    multi =
      ...
      |> Ecto.Multi.run(:log, fn %{release: release} -> audit(user, "release.publish", {package, release}) end)
    ...
    case HexWeb.Repo.transaction(multi) do
      {:ok, %{release: release}} ->
        ...
        release = %{release | package: package}
        release = HexWeb.Repo.preload(release, :requirements)
